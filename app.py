...
historico_passagens, ranking_atual_salvo = {}, []
status_prova, tipo_sessao, relogio_inicial_texto = "AGUARDANDO", "CLASSIFICACAO", "00:00"
...
<div style="display:flex;gap:20px;justify-content:center;margin-top:20px"><div class="box"><span>MODO:</span><select id="tipo-sessao" onchange="document.getElementById('lbl').innerText=this.value==='CLASSIFICACAO'?'MINUTOS':'VOLTAS'"><option value="CLASSIFICACAO">CLASSIFICAÇÃO</option><option value="CORRIDA">CORRIDA</option></select><input type="number" id="p-prova" value="5" style="width:50px;text-align:center"><span id="lbl">MINUTOS</span><button onclick="fetch('/cmd?acao=iniciar&valor='+document.getElementById('p-prova').value+'&sessao='+document.getElementById('tipo-sessao').value)" style="background:#00ff00;color:#000">ABRIR</button><button onclick="if(confirm('Encerrar?'))fetch('/cmd?acao=finalizar')" style="background:#ff4500;color:#fff">FIM</button><button onclick="if(confirm('Resetar?'))fetch('/cmd?acao=resetar')" style="background:#ff0000;color:#fff">RESET</button></div>
...
document.getElementById('cronometro').innerText=d.relogio;document.getElementById('status-label').innerText=d.status;const t=document.getElementById('tabela-tempos');if(d.ranking.length===0){t.innerHTML='<tr><td colspan="6">Pista aberta. Aguardando...</td></tr>';return}
...
def processar_dados_serial():
	...
	try:
		...
		while True:
			...
			if linha.startswith("KART:") and status_prova in ["CLASSIFICACAO", "CORRIDA", "BANDEIRADA"]:
...
def obter_ranking():
	global tipo_sessao, parametro_prova, relogio_inicial_texto, status_prova
	ranking = []
	...
	ranking_ordenado = sorted(ranking, key=lambda x: x["melhor_bruto"]) if tipo_sessao == "CLASSIFICACAO" else sorted(ranking, key=lambda x: (-x["voltas"], x["ultimo_timestamp"]))
...
@app.route('/cmd')
def cmd():
	global status_prova, tempo_limite_prova, tempo_inicio_prova, historico_passagens, relogio_inicial_texto, tipo_sessao, parametro_prova, vencedor_detectado
	acao = request.args.get('acao')
	if acao == 'iniciar' and status_prova == "AGUARDANDO":
		parametro_prova, tipo_sessao, vencedor_detectado = int(request.args.get('valor', 5)), request.args.get('sessao', 'CLASSIFICACAO'), False
		historico_passagens.clear()
		if tipo_sessao == "CLASSIFICACAO": tempo_limite_prova, tempo_inicio_prova, relogio_inicial_texto = parametro_prova * 60, time.time(), f"{parametro_prova:02d}:00"
		if tipo_sessao != "CLASSIFICACAO": relogio_inicial_texto = f"V. 0/{parametro_prova}"
		status_prova = tipo_sessao
	if acao == 'finalizar' and status_prova in ["CLASSIFICACAO", "CORRIDA", "BANDEIRADA"]:
		status_prova = "FINALIZADO"
		agora = datetime.now()
...
def falso():
	...
	if status_prova in ["CLASSIFICACAO", "CORRIDA", "BANDEIRADA"]:
		if id_kart not in historico_passagens: historico_passagens[id_kart] = []
